<template>
    <header>
        <nav>
            <h2>hello</h2>
            <button v-if="!isLoggedIn" @click="emit('sign-in')">Sign In</button>
            <div class="signed" v-else> 
                <div @click="$emit('upload')" class="upload">
                ...
                <div  class="profile">
                    <div class="tooltip">
                        ...
                        <img 
                            class="profile-img"
                            :src="`https://avatars.dicebear.com/api/pixel-art/${currentUser.name}.svg`" 
                            alt="profile-pic"
                            @click="emit('sign-out')"
                        >
                    </div>
...
<script setup>
    ...
    const emit = defineEmits(['sign-in', 'sign-out', 'upload'])
